...
from models import Aluno, Mensalidade, Frequencia, Conteudo, Matricula, ProgressoAula
...
def conteudo_aluno():
    ...
    conteudos= []
    if matricula:
        conteudos = (db.session.query(Conteudo, ProgressoAula)
            .outerjoin(ProgressoAula, (ProgressoAula.conteudo_id==Conteudo.id) &
                       (ProgressoAula.aluno_id==aluno.id))
            .filter(Conteudo.materia_id.in_(
                db.session.query(db.func.distinct(Conteudo.materia_id))))
            .order_by(Conteudo.data).all())
...
def concluir_aula(conteudo_id):
    ...
    if not p:
        p = ProgressoAula(aluno_id=session["aluno_id"], conteudo_id=conteudo_id, concluido=1)
        db.session.add(p)
    else:
        p.concluido = 1
    db.session.commit()
